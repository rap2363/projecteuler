...
public final class Problem59 implements EulerProblem {
    ...
    public Problem59() {
        ...
        try (final Scanner scanner = new Scanner(englishWordsFile)) {
            while (scanner.hasNextLine()) {
                this.englishWords.add(scanner.nextLine());
            }
            scanner.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) {
        System.out.println(charsToWords(decryptCipher(CIPHER_TEXT, new char[]{'g', 'o', 'd'})));
        System.out.println(new Problem59().run());
    }
    ...
    public long run() {
        ...
        for (char first = LOWER_CASE_A; first <= LOWER_CASE_Z; first++) {
            for (char second = LOWER_CASE_A; second <= LOWER_CASE_Z; second++) {
                for (char third = LOWER_CASE_A; third <= LOWER_CASE_Z; third++) {
                    final char[] key = new char[] {first, second, third};
                    final char[] decryptedCipher = decryptCipher(CIPHER_TEXT, key);
                    final DecryptedResult decryptedResult = new DecryptedResult(key, decryptedCipher, englishWords);
                    topResults.add(decryptedResult);
                    while (topResults.size() > MAX_SIZE_TO_COMPARE) {
                        topResults.remove(topResults.last());
                    }
                }
            }
        }

        // Hopefully the top one is the best representation
        final char[] decryptedCipher = topResults.first().decryptedCipher;
        long total = 0L;
        for (final char c : decryptedCipher) {
            total += c;
        }
        return total;
    }

    private static List<String> charsToWords(final char[] chars) {
    ...
    private static char[] decryptCipher(final char[] cipherText, final char[] key) {
        final char[] decryptedText = new char[cipherText.length];
        int keyIndex = 0;
        for (int i = 0; i < cipherText.length; i++) {
            decryptedText[i] = (char) (key[keyIndex] ^ cipherText[i]);
            keyIndex++;
    ...
    private static class DecryptedResult {
        ...
        public char[] key;
        public char[] decryptedCipher;
        public final double matchScore;

        public DecryptedResult(final char[] key,
                               final char[] decryptedCipher,
                               final Set<String> dictionary) {
            this.text = charsToWords(decryptedCipher);
            this.key = key;
            this.decryptedCipher = decryptedCipher;
            this.matchScore = calculateMatchScore(dictionary, text);
        }

        private double calculateMatchScore(final Set<String> dictionary, final List<String> text) {
            int count = 0;
            ...
            return text.size() > 0 ? ((double) count / text.size()) : 0d;
        }

        public static Comparator comparator = new Comparator<DecryptedResult>() {
            @Override
            public int compare(final DecryptedResult o1, final DecryptedResult o2) {
                return Double.compare(o2.matchScore, o1.matchScore);
            }
        };
    }
}
